...
{
    // 260626Codex: Generate DTSA-II no-GUI scripts that work with the dtsa2.msi app jar alone.
    internal sealed class SimulationScriptGenerator
    {
        ...
        public string Generate(SimulationProperty property, int? parallelIndex = null)
        {
            ...
            AppendBodyLine(builder, "det = create_detector(" + ToPythonString(property.DetectorName) + ")");
        ...
        // 260626Codex: Create an in-memory SDD so parallel msi jobs don't contend for DTSA-II's detector database.
        private static void AppendJavaApiHelpers(StringBuilder builder)
        {
            AppendBodyLine(builder, "def create_detector(name):");
            AppendBodyLine(builder, "\tdetector_name = name if name else \"MineraScope SDD\"");
            AppendBodyLine(builder, "\tdet = epd.EDSDetector.createSDDDetector(2048, 10.0, 0.0, 126.0)");
            AppendBodyLine(builder, "\tdet.getDetectorProperties().setName(detector_name)");
