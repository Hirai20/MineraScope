...
{
    // 260507Codex: manifest.json の保存場所決定と読み書きを一か所に集約します。
    internal sealed class SpectrumPoolRepository
    {
        ...
        // 260507Codex: 鉱物名と conditionKey から pool フォルダを決め、ファイル名に使えない文字を除きます。
        ...
        // 260507Codex: 既存 manifest があれば読み、なければ新方式の空 manifest を作ります。
        public SpectrumPoolManifest LoadOrCreate(SpectrumPoolHandle handle)
        {
            if (File.Exists(handle.ManifestPath))
            {
                var manifest = Load(handle.ManifestPath);
        ...
        // 260507Codex: 実行結果反映では handle がなくても manifest path から読み直せるようにします。
        ...
        // 260507Codex: pool フォルダを作成してから manifest を保存します。
        ...
        // 260507Codex: 実行結果反映時の manifest 保存も同じ JSON 設定に揃えます。
        ...
        // 260507Codex: Completed でも実ファイルがないものは Missing に落とし、学習対象から外します。
        public bool RefreshCompletedStatuses(SpectrumPoolHandle handle, SpectrumPoolManifest manifest)
        {
            ...
            foreach (var entry in manifest.Spectra.Where(entry => entry.Status == SpectrumManifestStatus.Completed))
            {
                ...
                // 260508Codex: manifest に残る理由はユーザーが読める自然な日本語にします。
        ...
        // 260507Codex: ファイルシステム上で安全に使える名前へ鉱物名を変換します。
    ...
    // 260507Codex: pool の場所と生成条件 snapshot を一緒に運ぶ軽量ハンドルです。
